...
def signed(x): return f'{x:.1f}'.replace('-','\u2212')
...
main_tables+=table('table3','Table 3. Prevalence ratios for any cost-related barrier among US stroke survivors under sequential adjustment',['Characteristic','Model 1, PR (95% CI)','Model 2, PR (95% CI)','Model 3, PR (95% CI)'],rows,'Survey-weighted modified Poisson regression with ten income imputations; all models include the same 6,777 survivors (1,117 with a barrier). Model 1 adjusts for survey year, age (spline), sex, race and Hispanic origin, and region. Model 2 adds education, family income, and insurance. Model 3 adds hypertension, diabetes, coronary heart disease, smoking, self-rated health, and disability. The age-group estimate is from a parallel model that replaces the age spline with an age-group indicator. Private coverage (with or without public coverage) is the reference for insurance. A dash indicates that the variable was not in the model. FPL indicates federal poverty level; GED, General Educational Development; PR, prevalence ratio. Complete Model 3 estimates are given in Table S8.')

supp=''
flow=pd.read_csv(ROOT/'outputs/cohort_flow.csv');poolflow=pd.read_csv(ROOT/'outputs/pooled_cohort_flow.csv').set_index('year')
rows=[[int(r.year),*[f'{int(x):,}' for x in (r.stroke_eligible,r.primary_observed,poolflow.loc[r.year,'stroke'],poolflow.loc[r.year,'primary_observed'],poolflow.loc[r.year,'primary_events'])]] for _,r in flow.iterrows()]
supp+=table('tableS3','Table S3. Annual and pooled sample counts by survey year',['Year','Annual stroke survivors','Annual, outcome observed','Pooled stroke survivors','Pooled, outcome observed','Pooled, with barrier'],rows,'Annual stroke records total 7,555; pooled stroke records total 7,181. The difference comes from excluding 2020 followback records in pooled analyses. These are unweighted counts.')
missing=pd.read_csv(OUT/'covariate_missingness.csv')
misslabels={'any_barrier':'Primary outcome (any cost-related barrier)','age':'Age','sex':'Sex','race':'Race and Hispanic origin','region':'Region','education':'Education','insurance':'Insurance','hypertension':'Hypertension','diabetes':'Diabetes','coronary':'Coronary heart disease','fairpoor':'Self-rated health','smoking':'Smoking','disability':'Disability'}
supp+=table('tableS4','Table S4. Missing outcome and nonincome covariate values in the pooled stroke-survivor sample',['Variable','No. missing','Missing, %'],[[misslabels.get(r.variable,r.variable),int(r.missing),f'{r.pct_missing:.2f}'] for _,r in missing.iterrows()],'Denominator is 7,181 stroke records. Missingness overlaps across variables. Among records with observed primary outcomes, 327 had at least one missing adjustment variable.')
supp+=table('tableS5','Table S5. Complete characteristics of the pooled sample, overall and by cost-related barrier status',['Characteristic','Overall (N=7,104)','No barrier (n=5,920)','Any barrier (n=1,184)'],characteristic_rows(list(varlabels)),t1note.replace(' Additional characteristics are given in Table S5.','')+' AIAN, American Indian or Alaska Native. The race/Hispanic-origin categories follow the published public-use recode.')
out_labels={'any_barrier':'Any cost-related barrier','universal_barrier':'Universal three-question composite','delayed_care':'Delayed medical care','forgone_care':'Forgone medical care','forgone_rx':'Forgone needed prescriptions','rx_underuse':'Medication underuse among users'}
supp+=table('tableS6','Table S6. Pooled prevalence of the composite outcome and each component barrier, 2019–2025',['Outcome','No. with observed outcome','No. with barrier','Prevalence, % (95% CI)'],[[out_labels[o],f'{int(r.n):,}',f'{int(r.events):,}',shortpct(r)] for o,r in components.iterrows()],'Components overlap. Medication-underuse estimates use prescription users only. Each prevalence denominator includes all stroke records with a classifiable corresponding outcome; it is not restricted to complete model covariates. All displayed estimates pass the implemented precision screen.')
...
t_m3=table('tableS8','Table S8. Fully adjusted (Model 3) prevalence ratios for all covariates',['Characteristic','PR (95% CI)','P value'],rows,'The model includes an age spline with 3 degrees of freedom. Spline basis coefficients and the intercept are provided in the public repository. No multiplicity correction was applied.')
...
supp+=table('tableS7','Table S7. Unadjusted pooled prevalence of any cost-related barrier in principal subgroups',['Subgroup','No. with observed outcome','Prevalence, % (95% CI)'],rows,'Income counts are averages across ten imputations. The unknown-insurance subgroup (29 records) failed the precision screen, and its prevalence is not displayed. These are unadjusted subgroup estimates.')
supp+=t_m3+t_sens+t_log
rows=[]
modellabels={'M1':'Model 1','M2':'Model 2','M3':'Model 3','Age_M1':'Age-group Model 1','Age_M2':'Age-group Model 2','Age_M3':'Age-group Model 3','AgeInteraction':'Year by age-group interaction',**senslabels}
for m,g in diag.groupby('model',sort=False):rows.append([modellabels.get(m,m),f'{int(g.n.iloc[0]):,}',f'{int(g.events.iloc[0]):,}',f'{g.above_one.min()}–{g.above_one.max()}',f'{g.max_fitted.max():.3f}','All converged'])
supp+=table('tableS11','Table S11. Convergence and fitted-value diagnostics for the modified Poisson models',['Model','No.','No. with barrier','Fitted values >1 per imputation','Largest fitted value','Convergence'],rows,'Ten fits per specification (one per income imputation). The bounded logistic models (Table S10) give similar associations. Age-group models replace the age spline with an age-group indicator.')

outcome_definitions=table('tableS1','Table S1. Outcome definitions and NHIS source variables',['Measure','NHIS variables','Universe'],[['Stroke history','STREV_A','All sample adults'],['Delayed medical care','MEDDL12M_A','All sample adults'],['Forgone medical care','MEDNG12M_A','All sample adults'],['Forgone needed prescriptions','RXDG12M_A','All sample adults'],['Prescription use','RX12M_A','All sample adults'],['Dose skipping, dose reduction, delayed filling','RXSK12M_A; RXLS12M_A; RXDL12M_A','RX12M_A=1'],['Any affordability barrier','Any applicable positive among six items','Known applicability and response rules described below']])
covariate_definitions=table('tableS2','Table S2. Covariate definitions and NHIS source variables',['Domain','Variables','Analysis categories'],[['Age','AGEP_A','18–85; 85 is top-coded; natural spline or 18–64/65+'],['Sex','SEX_A','Male; female'],['Race/Hispanic origin','HISPALLP_A','Seven published categories'],['Region','REGION','Northeast; Midwest; South; West'],['Education','EDUC_A; EDUCP_A','Four harmonized attainment groups'],['Income','RATCAT_A; IMPNUM or IMPNUM_A','<100%; 100–199%; 200–399%; ≥400% FPL'],['Insurance','NOTCOV_A and seven coverage indicators','Private; public without private; military only; uninsured'],['Clinical history','HYPEV_A; DIBEV_A; CHDEV_A','Yes/no hypertension, diabetes, coronary disease'],['Smoking','SMKCIGST_A','Never; former; current; indeterminate missing'],['Self-rated health','PHSTAT_A','Fair/poor vs good/very good/excellent'],['Disability','DISAB3_A','Published Washington Group disability recode']])
strobe_rows=[['1','Title and Abstract','Design, population, period and principal quantitative findings.'],['2–3','Introduction','Background, prior studies and objectives stated.'],['4–5','Methods / Study design','Repeated cross-sectional design, public NHIS setting and 2019–2025 period.'],['6','Methods / Population; Figure S1','Eligibility, stroke definition, age exclusion, annual vs pooled selection.'],['7–8','Methods / Outcomes and covariates; Tables S1–S2','Definitions, universes and cross-year source verification; crosswalk files included.'],['9','Discussion / Limitations','Self-report, nonresponse, institutional exclusion, confounding and missingness addressed.'],['10','Methods / Population','All eligible records in the selected years; no formal power-based sample selection.'],['11','Methods / Covariates','Age spline and categorical income/education definitions given.'],['12','Methods / Statistical analysis; Supplemental Methods','Survey weighting, MI, missingness, interaction and sensitivity procedures.'],['13','Results / Study population; Table S3; Figure S1','Counts and reasons for exclusions; different annual and pooled paths.'],['14','Table 1; Tables S4–S5','Characteristics and missingness; income counts averaged across imputations.'],['15','Table 2; Tables S6–S7','Outcome counts and annual, component and subgroup estimates.'],['16','Table 3; Table S8; Results','Adjusted PRs, covariate sets, precision and absolute annual difference.'],['17','Results / Sensitivity analyses; Tables S9–S11','Sensitivity estimates and model diagnostics.'],['18–21','Discussion and Conclusions','Key results, limitations, cautious interpretation and generalizability.'],['22','Declarations','No external funding is reported.']]
strobe=table('tableS12','Table S12. STROBE checklist for cross-sectional studies',['Item','Location in manuscript','Content'],strobe_rows,'')
...
def cite(r):
    ...
    if r.get('group'):authors+='; '+r['group']
    if r.get('journal'):
...
tokens={'pooled':pct(components.loc['any_barrier']),'young':pct(sg.loc['young','18-64']),'older':pct(sg.loc['young','65+']),
 ...
 'adj2019':pct(adj.loc[2019]),'adj2025':pct(adj.loc[2025]),'difference':f"{signed(100*difference.difference)} percentage points (95% CI, {signed(100*difference.lower)} to {signed(100*difference.upper)})",
...
for _k,_v in list(tokens.items()):
    if isinstance(_v,str) and ' (95% CI, ' in _v and _v.endswith(')'):tokens[_k+'_s']=_v[:-1].replace(' (95% CI, ','; 95% CI, ')
...
def write_docx(markdown,path,body_spacing=2.0,line_numbers=False,figure_width=6.5):
    ...
    break_before={'## Abstract','## Introduction','## References','## Tables','## Figures','## Supplemental Tables','## Supplemental Figure','## STROBE Checklist'}
    while i<len(lines):
        line=lines[i].strip()
        ...
        if is_note:
            ...
            if i<=4 and not line.startswith(('^','*')):p.alignment=WD_ALIGN_PARAGRAPH.CENTER
            inline(p,line)
        else:
            inline(p,line)
        last_kind='para';i+=1
